...
function solveBoard(board, beams) {
  ...
  while (beamsActive && !distanceLimitReached) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      if (
        beams[i].row < 0 ||
        beams[i].row >= board.length ||
        beams[i].col < 0 ||
        beams[i].col >= board[beams[i].row].length
      ) {
...
function part2() {
  ...
  for (let i = 0; i < board.length; i++) {
    ...
    let beams = [
      { row: i, col: 0, direction: "E", isActive: true, distance: 0 },
    ];
  ...
  for (let i = 0; i < board[0].length; i++) {
    let newBoard = JSON.parse(JSON.stringify(board));
    let beams = [
      { row: 0, col: i, direction: "S", isActive: true, distance: 0 },
    ];
    solveBoard(newBoard, beams);
    energizedTestResults.push(calculateTotalEnergized(newBoard));
  }
